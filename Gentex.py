...
class hold:
	def __init__(self,foils):
		...
		foil = open("json.property", "w+")
		foil.write(f"version={self.info['me']['keywords']}")
		foil.close()
	...
	def render(self):
		if (self.latex):
			...
			for key, value in self.info.items():
				if key not in ['me','newTech', 'interests', 'selectGroups', 'selectProjects', 'research']:
...
class website:
	...
	def convertItem(key, value):
		return {
			...
			"mentor": website._mentee,
			"submissions": website._submission
		}[key](value)
	...
	def _submission(sub):
		return {
			...
			"date": sub['Date'],
			"conf": sub['Conf'],
			"conf_URL": sub['Conf_URL'],
			"url": sub['URL'],
			"type": sub['Type'],
			"author": sub['Author'],
			"localurl":None if 'localurl' not in sub else sub['localurl'],
			"sig":None if 'sig' not in sub else sub['sig']
		}
	...
	def _mentee(mentee):
		...
		if "FERPA" in mentee and mentee['FERPA']:
			name = mentee['name']
			exp = mentee['exp'] + " in " + mentee['degree']

		return {
			"name": name,
			"exp": exp,
			"dates": website.niceDates(mentee['start'], mentee['end']),
			"about": website.itemizeList(mentee['about'])
		}
